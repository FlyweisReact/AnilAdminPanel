...
const EProduct = () => {
  ...
  function MyVerticallyCenteredModal2(props) {
    ...
    const fetchProduct = async () => {
      try {
        const { data } = await axios.get(` https://xw9mf13nwl.execute-api.ap-south-1.amazonaws.com/dev/api/v1/products/${id}`)
        setProduct(data)
      }catch(e) { 
        console.log(e)
      }
    ...
    return (
      ...
      >
        ...
        <Modal.Body>
          ...
          <p className="View">
            {" "}
            <strong>Available : </strong> {product?.data?.isAvailable === true ? "Yes" : "No"}
          </p>
          <p className="View">
            {" "}
          ...
          <p className="View">
            {" "}
            <strong>Discount percentage : </strong> {product?.data?.discountPercent}%
          </p>
         
          <p className="View">
            {" "}
            <strong>Discounted Price : </strong> ₹{product?.data?.discountedPrice}
          </p>
            ...
            <tbody>
              {data?.data?.map((i, index) => (
                <tr key={index}>
                  ...
                  <td> {i.name} </td>
                  <td> ₹{i.price} </td>
                  <td style={{textDecoration : 'line-through'}} > ₹{i.discountedPrice} </td>
                  <td>
                    {i.stock > 10 ? (
