...
const EProduct = () => {
  ...
  const npage = Math.ceil(21 / recordsPerPage )
  ...
  const deleteHandler = async (id) => {
    try {
      const { data } = await axios.delete(` https://xw9mf13nwl.execute-api.ap-south-1.amazonaws.com/dev/api/v1/products/${id}`)
      console.log(data)
      fetchData()
  ...
  return (
    <>
      ...
      >
        ...
        >
          All Product's (Total : {data?.data?.length})
        </span>
        <button
          onClick={() => {
            setEdit(false);
        ...
        <div className="overFlowCont">
          ...
          <ul>
            ...
            </li>
            {
              numbers?.map((n , i) => (
                <li>
                  <a href="#" onClick={changeCPage} key={i} > {n} </a>
                </li>
              ))
            }
            <li>
