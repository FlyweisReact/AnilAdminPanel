...
const EProduct = () => {
  ...
  // if(query) {
  //   // setCurrentPage(1);
  //   setFilterData(data?.filter((i) => i.name?.toLowerCase().includes(query?.toLowerCase()))) ;
  // }else{
  ...
  for (let i = 1; i <= Math.ceil((!query ? dataLength : filterData?.length) / postPerPage); i++) {
    pages.push(i);
  }
