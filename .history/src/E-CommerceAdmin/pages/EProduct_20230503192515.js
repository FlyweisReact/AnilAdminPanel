...
const EProduct = () => {
  ...
  const serachData = filterData?.slice(firstPostIndex, lastPostIndex);
  const searchBy = !query ? currentPosts : serachData;

  let pages = [];
  for (
    let i = 1;
    i <= Math.ceil((!query ? dataLength : filterData?.length) / postPerPage);
    i++
  ) {
    pages.push(i);
  }
...
// const currentPosts2 = data?.slice(firstPostIndex, lastPostIndex);

const filterData2 =  data?.filter((i) => i.name?.toLowerCase().includes(query?.toLowerCase()));

// const serachData2 = filterData2?.slice(firstPostIndex2, lastPostIndex2);
// const searchBy2 = !query ? currentPosts2 : serachData2;

let pages2 = [];

const TotolData = filterData2 ? filterData2 : data
if(TotolData === filterData2) {
  setCurrentPage2(1)
}
...
for (
  let i = 1;
  i <= Math.ceil(TotolData?.length/ postPerPage2);
  i++
) {
  pages2.push(i);
}
            ...
            <Form.Group className="mb-3">
              ...
              >
                ...
                {brandData?.data?.map((i, index) => (
                  <option key={index} value={i.brandName}>
                    {" "}
                    {i.brandName}{" "}
                  </option>
                ))}
          ...
          <div className="pagination">
            ...
            {pages2?.slice(currentPage2 - 1, currentPage2 + 3).map((i, index) => (
              <button
                ...
                className={currentPage2 === i ? "active" : ""}
              >
                {" "}
                {i}{" "}
              </button>
